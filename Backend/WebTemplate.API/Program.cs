...
// Build the web application (top-level statements for testability)
...
{
    // Ensure the server is started for WebApplicationFactory/TestServer scenarios
    await app.StartAsync();
}

static void BuildConfiguration(IConfigurationBuilder configurationBuilder, IWebHostEnvironment env)
{
    var processName = Process.GetCurrentProcess().ProcessName?.ToLowerInvariant() ?? string.Empty;
...
// Expose an entry-point type for WebApplicationFactory
