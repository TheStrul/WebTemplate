...
try {
    ...
    if ($NonInteractive) {
        $projectInfo = Get-ProjectInformation -NonInteractive -ProjectName $ProjectName -TargetPath $TargetPath
    } else {
        $projectInfo = Get-ProjectInformation -ProjectName $ProjectName -TargetPath $TargetPath
    }
    ...
    Copy-TemplateFiles -TemplatePath $projectInfo.TemplatePath -TargetPath $projectInfo.TargetPath -IncludeFolders $projectInfo.IncludeFolders -Exclusions $projectInfo.Exclusions
    ...
    if (-not $SkipGit) {
        ...
        if ($NonInteractive) {
            Initialize-GitRepository -TargetPath $projectInfo.TargetPath -ProjectName $projectInfo.NewName -NonInteractive
        } else {
