...
function Update-WorkspaceConfigs {
    param(
        [string]$TargetPath,
        [hashtable]$ProjectInfo
    )

    Write-Info "Updating workspace configuration files..."

    # Clean path - remove any quotes
    $TargetPath = $TargetPath.Trim('"').Trim("'")

    $configFiles = Get-ChildItem -Path (Join-Path $TargetPath "Configurations") -Filter "workspace-config.json" -Recurse -ErrorAction SilentlyContinue

    foreach ($configFile in $configFiles) {
        try {
            $json = Get-Content $configFile.FullName -Raw | ConvertFrom-Json

            # Update initScriptPath in database configuration
            if ($json.PSObject.Properties['database'] -and $json.database.PSObject.Properties['initScriptPath']) {
                $oldPath = $json.database.initScriptPath
                $newPath = $oldPath -replace [regex]::Escape($ProjectInfo.OldName), $ProjectInfo.NewName
                $json.database.initScriptPath = $newPath
                Write-Info "Updated initScriptPath: $oldPath -> $newPath"
            }

            # Save updated JSON
            $json | ConvertTo-Json -Depth 100 | Set-Content -Path $configFile.FullName -NoNewline
            Write-Success "Updated $($configFile.Name)"
        }
        catch {
            Write-Warning "Failed to update $($configFile.FullName): $_"
        }
    }

    Write-Success "Updated workspace configuration files"
}
